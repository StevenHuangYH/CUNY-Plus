...
const CUNYPlusButton = ({ anchor }: PlasmoCSUIProps) => {
  const {
    ...
    search,
    select
  } = useProfessorLookup(anchor?.element)
  const name = course.professorName
  const error = result?.status === "error" ? result.error : null
  const searchUrl = `https://www.ratemyprofessors.com/search/professors?q=${encodeURIComponent(name)}`
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      if (rootNode && rootNode.host) {
        ;(rootNode.host as HTMLElement).style.setProperty(
          "z-index",
          "9999",
          "important"
        )
  ...
  useEffect(() => {
    ...
    const adjustPosition = () => {
      if (cardRef.current && containerRef.current) {
        ...
        if (
          containerRect.bottom + cardRect.height + 10 > window.innerHeight &&
          containerRect.top - cardRect.height - 10 > 0
        ) {
  ...
  }, [showCard, rating, result, loading])

  // Gracefully skip rendering buttons for empty, Staff, or TBA slots
  const isInvalidName = isPlaceholder(name)
  if (isInvalidName) {
    return null
  }
  ...
  const handleButtonClick = (e: React.MouseEvent) => {
    ...
    if (result && result.status !== "error") setShowCard(true)
    else search()
  }
  ...
  return (
    <div
      ...
      <button
        type="button"
        aria-label={`View RateMyProfessors ratings for ${name}`}
        ...
        onMouseEnter={(e) => {
          if (!loading)
            (e.currentTarget as HTMLButtonElement).style.background = NAVY_DARK
        }}
        onMouseLeave={(e) => {
          if (!loading)
        ...
        createPortal(
          <div
            ...
            style={{
              ...
              top: cardPos.top,
              left: cardPos.left,
              ...(cardPos.marginTop !== undefined
                ? { marginTop: cardPos.marginTop }
                : {}),
              width: "max-content",
              minWidth: 240,
              maxWidth: 280,
            ...
            <div
              ...
              {loading ? (
                <p role="status">Searching professors…</p>
              ) : error ? (
                <>
                  <div
                    style={{
                ...
                <>
                    ...
                    <div
                      ...
                      <p
                        style={{
                          ...
                          fontSize: 16,
                          fontWeight: 700,
                          color: getDifficultyColor(rating.avgDifficulty)
                        }}>
                  ...
                  <div
                    ...
                    <span style={{ fontWeight: 500 }}>
                      {rating.numRatings === undefined
                        ? "Review count unavailable"
                        : `Based on ${rating.numRatings} reviews`}
                    </span>
                    {rating.legacyId && (
                      <a
                  ...
                  <ul
                    ...
                    {result.candidates.map((candidate) => (
                      <li
                        ...
                        <button
                          type="button"
                          aria-label={`Show ratings for ${candidate.name} at ${candidate.schoolName}`}
